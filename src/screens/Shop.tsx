...
function Shop() {
  ...
  return (
     <section className='my-15 px-8 sm:px-10 xl:px-20'>
          ...
          <p className='text-gray-500 text-md mb-8'>Browse our curated collection of premium products.</p>

          <div className='flex items-center flex-wrap gap-3'>
              {products.map(each =>
                 <Button className={activeCategory === each.category ? "active-category":"filter-btn"} onClick={()=>setCategory(each.category)} type="primary" key={each.id}>{each.category}</Button>
              )}
          </div>
